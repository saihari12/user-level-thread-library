...
foothread_mutex_t forend;             // Mutex for end
...
int main()
{
    ...
    // foothread_barrier_init(&barrier, num_of_leaf);
    ...
    foothread_mutex_destroy(&common);
    foothread_mutex_destroy(&forend);
...
void *node_function(void *arg)
{
    ...
    foothread_mutex_lock(&forend);
    foothread_exit();
    foothread_mutex_unlock(&forend);
